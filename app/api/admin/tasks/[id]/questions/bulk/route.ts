...
export async function POST(
  ...
  if (body.bankId) {
    ...
    if (bank.items.length === 0) return NextResponse.json({ error: "没有可导入的题目" }, { status: 400 });
    ...
    if (dataRows.length === 0) {
      return NextResponse.json({ error: "CSV 中没有有效数据行" }, { status: 400 });
    }
    ...
    if (lines.length === 0) {
      return NextResponse.json({ error: "没有有效题目" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "请提供 text、csv 或 bankId 字段" }, { status: 400 });
  }
